<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT id, countryName,shortDesc FROM country LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
				while($data = $sql->fetch_array()) {
					$response .= '
						<tr>
							...
							<td id="country_'.$data["id"].'">'.$data["shortDesc"].'</td>
		...
		if ($_POST['key'] == 'addNew') {
			...
			if ($sql->num_rows > 0)
				...
				$result = $conn->query("INSERT INTO country (countryName, shortDesc, longDesc) 
